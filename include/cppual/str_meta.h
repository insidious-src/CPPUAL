...
template <symbolic_char T = char>
constexpr std::size_t char_hash (T const* input, std::size_t sz = std::size_t (-1)) noexcept
{
    constexpr static const auto npos = static_cast<decltype (sz)> (-1);

    return (sz == npos || sz-- > 0) && *input ?
            decltype (sz) (*input) + 33 * char_hash (input + 1, sz) : 5381;
}
...
template <auto const* In, std::size_t SZ = std::size_t (-1)>
...
consteval std::size_t char_hash () noexcept
{
    ...
    return (SZ == npos || SZ > 0) && *In ?
            decltype (SZ) (*In) + 33 * char_hash<In + 1, SZ == npos ? SZ : SZ - 1> () : 5381;
}
