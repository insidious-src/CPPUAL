...
template <typename T>
using remove_cvrefptr_t = std::remove_cvref_t<std::remove_pointer_t<T>>;
...
/// lambda concepts
template <typename T>
concept lambda_non_capture_t = std::is_constructible_v<
        member_function_to_static_t<decltype (&std::decay_t<T>::operator())>, T>;

template <typename T>
concept lambda_capture_t = !std::is_constructible_v<
        member_function_to_static_t<decltype (&std::decay_t<T>::operator())>, T>;
